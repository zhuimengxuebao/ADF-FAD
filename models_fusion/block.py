...
class Block(nn.Module):
    def __init__(self, config, vis, mm=False):
        ...
        self.attention_norm = LayerNorm(config.data_hidden_size, eps=1e-6)
        self.ffn_norm = LayerNorm(config.data_hidden_size, eps=1e-6)
        if mm:
            self.att_norm_text = LayerNorm(config.data_hidden_size, eps=1e-6)
            self.ffn_norm_text = LayerNorm(config.data_hidden_size, eps=1e-6)
