...
class Attention(nn.Module):
    def __init__(self, config, vis, mm=True):
        ...
        self.attention_head_size = int(config.data_hidden_size / self.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        self.query = Linear(config.data_hidden_size, self.all_head_size)
        self.key = Linear(config.data_hidden_size, self.all_head_size)
        self.value = Linear(config.data_hidden_size, self.all_head_size)

        if mm:
            self.query_text = Linear(config.data_hidden_size, self.all_head_size)
            self.key_text = Linear(config.data_hidden_size, self.all_head_size)
            self.value_text = Linear(config.data_hidden_size, self.all_head_size)
            self.out_text = Linear(config.data_hidden_size, config.data_hidden_size)
        ...
        self.out = Linear(config.data_hidden_size, config.data_hidden_size)
    ...
    def forward(self, hidden_states, text=None):
        ...
        if text is None:
            ...
            context_layer_img = context_layer_img.permute(0, 2, 1).contiguous()
            context_layer_text = torch.matmul(attention_probs_text, value_layer_text)
            context_layer_text = context_layer_text.permute(0, 2, 1).contiguous()
            context_layer_it = torch.matmul(attention_probs_it, value_layer_text)
            context_layer_it = context_layer_it.permute(0, 2, 1).contiguous()
            context_layer_ti = torch.matmul(attention_probs_ti, value_layer_img)
            context_layer_ti = context_layer_ti.permute(0, 2, 1).contiguous()
